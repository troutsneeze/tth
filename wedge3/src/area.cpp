...
bool Area::start()
{
	...
	if (new_game) {
		...
		level_num = atoi(name.c_str());
		if (level_num == INSTANCE->get_num_levels()-1) {
			name = "end";
		}
		else if (level_num == 12 || level_num == 51 || level_num == 403 || level_num == 776) {
			name = "level404";
		}
		else if (level_num > 0 && ((level_num+1) % 10) == 0) {
			name = "level10";
		}
		else {
			name = "level";
